function realtime_neuralynx_acquisition(cfg)

% realtime_neuralynx_acquisition reads continuous data from a Neuralynx Cheetah
...
% Skipping this as the netcom package is not being detected as in installed
% package even though it has been referenced by matlab (path set)
...
if ~NlxAreWeConnected
    ...
    if succeeded ~= 1
      fprintf('FAILED connect to %s. Exiting script.\n', cfg.acquisition);
      return;
    else
...
if succeeded ~= 1
    fprintf('FAILED set the application name\n');
else
    fprintf('PASSED set the application name \n');
end
...
if succeeded == 0
    fprintf('FAILED get cheetah objects and types\n');
else
    fprintf('PASSED get cheetah objects and types\n');
end

%% open up a stream for all objects
for index = 1:length(cheetahObjects)
    if strcmp(char(cheetahObjects(index)), 'AcqSystem1') ~= 1 
        succeeded = NlxOpenStream(cheetahObjects(index));
        if succeeded == 0
            fprintf('FAILED to open stream for %s\n', char(cheetahObjects(index)));
            break;
        end
    end
end
...
if succeeded == 1
    fprintf('PASSED open stream for all current objects\n');
end
...
% Continously, try receiving data
while again
	pass = pass + 1;
	...
	for objectIndex = evtIndex(:)'
        ...
        if ~succeeded
            fprintf('FAILED to get new new events from stream %s on pass %d\n', objectToRetrieve, pass);
            break;
        elseif numRecordsReturned==0
            fprintf('FAILED to get new events from stream %s on pass %d\n', objectToRetrieve, pass);
            break;
        else
            fprintf('Retrieved %d new events from %s with %d dropped.\n', numRecordsReturned, objectToRetrieve, numRecordsDropped);
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	if any(recordComplete)
        ...
        if ~isempty(last)
            ...
            totalSamples                   = totalSamples + size(alldata, 2);
            if totalSamples / 10000 >= saveSamplesCounter
                record_name = sprintf('\\%d_record.mat', totalSamples);
                save(strcat(cfg.recordPoolPath, record_name), 'recordPool');
                saveSamplesCounter = saveSamplesCounter + 1;
            end
            clear alldata
        end
    end

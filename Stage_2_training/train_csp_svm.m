function train_csp_svm(cfg, re_cfg)
...
% Save memory
clearvars trian_X train_Y test_X test_Y;

% Find CSP filters
class_dat = {};
n_unique_classes = size(unique(all_Y), 1);
for i=1:n_unique_classes
    class_dat{i} = all_X(all_Y==i-1, :);
    class_dat{i} = reshape(class_dat{i}, [size(class_dat{i}, 1), nchan, re_cfg.length*cfg.hdr.Fs]);
end

fprintf('Finding CSP filters...\n');
w = find_csp(class_dat);
all_X = apply_filter([class_dat{1}; class_dat{2}], w, 2);
all_Y = [zeros(size(class_dat{1}, 1), 1); ones(size(class_dat{2}, 1), 1)];

% Shuffe train data
rand_indices = randperm(size(all_X, 1));
all_X = all_X(rand_indices, :);
all_Y = all_Y(rand_indices, :);

[SVMModel, FitInfo] = fitclinear(all_X, all_Y, 'Verbose', 0);
disp(FitInfo);
